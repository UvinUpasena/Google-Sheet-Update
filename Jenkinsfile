    ...
    stages {
        ...
        stage('Install Dependencies for Layer') {
            steps {
                ...
                docker run --rm -v "/var/lib/jenkins/workspace/lambda-deploy/layer":/var/task --entrypoint="/bin/sh" public.ecr.aws/lambda/python:3.13 -c "pip install -r /var/task/requirements.txt -t /var/task/python/lib/python3.13/site-packages/"
                '''
            }
        }
